create or replace view `steam_lakehouse`.`dbt_dev_silver`.`stg_steam__apps`
...
, 
  ...
  comment 'Una fila por appid del catalogo de Steam.'
...
with source as (
    ...
    select * from `steam_lakehouse`.`bronze`.`steam_app_list`
