create or replace view `steam_lakehouse`.`dbt_dev`.`alerts_dbt_source_freshness`
...
with results as (
  select * from `steam_lakehouse`.`dbt_dev`.`dbt_source_freshness_results`
),

sources as (
  select * from `steam_lakehouse`.`dbt_dev`.`dbt_sources`
)
