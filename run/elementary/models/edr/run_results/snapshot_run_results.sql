create or replace view `steam_lakehouse`.`dbt_dev`.`snapshot_run_results`
...
with dbt_run_results as (
    select * from `steam_lakehouse`.`dbt_dev`.`dbt_run_results`
),

dbt_snapshots as (
    select * from `steam_lakehouse`.`dbt_dev`.`dbt_snapshots`
)
